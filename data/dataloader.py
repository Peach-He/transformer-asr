...
from data.sampler import ReproducibleRandomSampler, DistributedSampler, DistributedSamplerWrapper
...
import data.dataio
import data.audio.processing.speech_augmentation


logger = logging.getLogger(__name__)

def make_dataloader(dataset, stage, looped_nominal_epoch=None, **loader_kwargs):
    if stage == 'train':
        loader_kwargs = train_loader_specifics(dataset, False, 0, loader_kwargs)
...
def train_loader_specifics(dataset, distributed_launch, rank, loader_kwargs):
...
#############################remove train dataset sorting
    ...
    def audio_pipeline(wav):
        sig = data.dataio.read_audio(wav)
        return sig

    data.dataset.add_dynamic_item(valtest_datasets, audio_pipeline)
    ...
    def audio_pipeline_train(wav):
        ...
        if hparams["speed_perturb"]:
            ...
            speed = data.audio.processing.speech_augmentation.SpeedPerturb(
    ...
    data.dataset.add_dynamic_item([train_data], audio_pipeline_train)
    ...
    data.dataset.add_dynamic_item(datasets, text_pipeline)

    # 4. Set output:
    data.dataset.set_output_keys(
